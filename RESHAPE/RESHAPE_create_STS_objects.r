...
if( !exists( "df_event_factor" ) )
    ...
    if( run_cohort_generator == "Y"){
      source( 'RESHAPE_cohort_generator.r' )
      if( !exists( 'df_log_PandT_longFormat_simplified_StrataLabels'  ) )
      {
        run_data_formatter <- readline("It looks like `RESHAPE_format_the_data.r has not been run\nDo you want to run `RESHAPE_format_the_data.r`? [Y, N]")
        if( run_data_formatter == "Y"){ source( 'RESHAPE_format_the_data.r' ) }
      }
    }
}
# ----
...
######################################
## Test-only state-sequence object. ##
######################################
# ----
#
...
##############################################
## Intervention-only state-sequence object. ##
##############################################
# ----
#
...
#########################################################
## Test-and-intervention status state-sequence object. ##
#########################################################
# ----
#
# Like the test-only objects, I actually make two. One contains the "Unobserved" state, which is useful for tracking
...
###################################### 
## HMA state state-sequence object. ##
######################################
# ----
...
################################################
## HMA-and-Test status state-sequence object. ##
################################################
# ----
#
# Like the test-only objects, I actually make two. One contains the "Unobserved" state, which is useful for tracking
...
###########################################################
## Test-and-multimorbidity status state-sequence object. ##
###########################################################
# ----
#

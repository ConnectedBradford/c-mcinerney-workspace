# RESHAPE_cohort_generator.R
#
# The purpose of this script is to create the sample cohort used in subsequent
# analyses.
#
...
#############################
# ----
source( "RESHAPE_study_parameters.R" )
# ----
...
project_id = "CB_2078"
qry_srcode <- dplyr::tbl( con, sql( "SELECT * FROM dbo.cb_SRCode" ) )
qry_srpatient <- dplyr::tbl( con, sql( "SELECT * FROM cb_SRPatient" ) )
qry_BNF_DMD_SNOMED_lkp <-
  dplyr::tbl( con, sql( "SELECT * FROM .lkp_BNF_DMD_SNOMED" ) )
qry_srprimarycaremedication <-
...
codes_BNF_meds_of_interest <- df_BNF_meds_of_interest$code
df_meds_names_simplified <-
  readr::read_csv( file = 'codelists/meds_names_simplified.csv',
                   col_types = cols( term = col_character(), term_simplified = col_character() ) )
# ## ## Amalgamate medicine preparations.
qry_names_meds_of_interest <-
  qry_BNF_DMD_SNOMED_lkp %>%
  dplyr::filter( BNF_Code %in% codes_BNF_meds_of_interest ) %>%
  dplyr::select( BNF_Code, DMplusD_ProductDescription )
vectorOf_names_meds_of_interest <-
  dplyr::pull( qry_names_meds_of_interest, DMplusD_ProductDescription )
df_shortNames_meds_of_interest <-
  qry_names_meds_of_interest %>%
  ...
  dplyr::collect() %>%
  dplyr::left_join(
    df_BNF_meds_of_interest %>% filter( nchar( code ) == 9 )
...
# If it doesn't exist, make a lazy table version of `df_shortNames_meds_of_interest`
# so that it can be used without collecting.
rm( qry_shortNames_meds_of_interest )
dplyr::copy_to( dest = con, df = df_shortNames_meds_of_interest, overwrite = TRUE )
qry_shortNames_meds_of_interest <-
  dplyr::tbl( con, sql( "SELECT * FROM #df_shortNames_meds_of_interest" ) )

# ## Multimorbidity codes.
codes_SNOMED_depression <-
  readr::read_csv(file = 'codelists/nhsd-primary-care-domain-refsets-depr_cod-20210127.csv',
                  col_types = cols( code = col_character(), term = col_character() ) )$code
codes_SNOMED_anxiety <-
  readr::read_csv(file = 'codelists/opensafely-anxiety-disorders-6aef605a.csv',
                  col_types = cols( code = col_character(), term = col_character() ) )$code
codes_SNOMED_substanceAbuse <-
  readr::read_csv(file = 'codelists/nhsd-primary-care-domain-refsets-illsub_cod-20210127.csv',
                  col_types = cols( code = col_character(), term = col_character() ) )$code
codes_SNOMED_dementia <-
  readr::read_csv(file = 'codelists/nhsd-primary-care-domain-refsets-dem_cod-20210127.csv',
                  col_types = cols( code = col_character(), term = col_character() ) )$code
codes_SNOMED_SMI <-
  readr::read_csv(file = 'codelists/nhsd-primary-care-domain-refsets-mh_cod-20210127.csv',
                  col_types = cols( code = col_character(), term = col_character() ) )$code
codes_SNOMED_arthritis <-
  readr::read_csv(file = 'codelists/nhsd-primary-care-domain-refsets-c19rarth_cod-20210127.csv',
                  col_types = cols( code = col_character(), term = col_character() ) )$code
codes_SNOMED_lungCancer <-
  readr::read_csv(file = 'codelists/opensafely-lung-cancer-snomed-2020-04-15.csv',
                  col_types = cols( id = col_character(), name = col_character() ) )$id
codes_SNOMED_otherCancer <-
  readr::read_csv(file = 'codelists/opensafely-cancer-excluding-lung-and-haematological-snomed-2020-04-15.csv',
                  col_types = cols( id = col_character(), name = col_character() ) )$id
codes_SNOMED_haemCancer <-
  readr::read_csv(file = 'codelists/nhsd-primary-care-domain-refsets-c19haemcan_cod-20210127.csv',
                  col_types = cols( code = col_character(), term = col_character() ) )$code
codes_SNOMED_hypertension <-
  readr::read_csv(file = 'codelists/nhsd-primary-care-domain-refsets-hyp_cod-20210127.csv',
                  col_types = cols( code = col_character(), term = col_character() ) )$code
codes_SNOMED_hyperlipid <-
  readr::read_csv(file = 'codelists/ciaranmci-hyperlipidaemia-68d4d269.csv',
                  col_types = cols( code = col_character(), term = col_character() ) )$code
codes_SNOMED_cardiacArrhythmia <-
  readr::read_csv(file = 'codelists/ciaranmci-cardiac-arrhythmias-4fc55f34.csv',
                  col_types = cols( code = col_character(), term = col_character() ) )$code
codes_SNOMED_asthma <-
  readr::read_csv(file = 'codelists/nhsd-primary-care-domain-refsets-ast_cod-20210127.csv',
                  col_types = cols( code = col_character(), term = col_character() ) )$code
codes_SNOMED_CAD <-
  readr::read_csv(file = 'codelists/ciaranmci-coronary-artery-disease-36b5ebff.csv',
                  col_types = cols( code = col_character(), term = col_character() ) )$code
codes_SNOMED_COPD <-
  readr::read_csv(file = 'codelists/nhsd-primary-care-domain-refsets-copd_cod-20210127.csv',
                  col_types = cols( code = col_character(), term = col_character() ) )$code
codes_SNOMED_osteo <-
  readr::read_csv(file = 'codelists/nhsd-primary-care-domain-refsets-osteo_cod-20200812.csv',
                  col_types = cols( code = col_character(), term = col_character() ) )$code
codes_SNOMED_CKD1and2 <-
  readr::read_csv(file = 'codelists/nhsd-primary-care-domain-refsets-ckd1and2atrisk1_cod-20200812.csv',
                  col_types = cols( code = col_character(), term = col_character() ) )$code
codes_SNOMED_CKD3to5 <-
  readr::read_csv(file = 'codelists/nhsd-primary-care-domain-refsets-ckdatrisk2_cod-20210127.csv',
                  col_types = cols( code = col_character(), term = col_character() ) )$code
codes_SNOMED_CHF <-
  readr::read_csv(file = 'codelists/ciaranmci-congestive-heart-failure-1da678ca.csv',
                  col_types = cols( code = col_character(), term = col_character() ) )$code
codes_SNOMED_hepatitis <-
  readr::read_csv(file = 'codelists/ciaranmci-hepatitis-04970595.csv',
                  col_types = cols( code = col_character(), term = col_character() ) )$code
...
codes_SNOMED_didNotAttend <-
  readr::read_csv(file = 'codelists/ciaranmci-did-not-attend-098119da.csv',
                  col_types = cols( code = col_character(), term = col_character() ) )$code
...
##########################
# ----
# This was trivial in previous iterations because I relied on clinically-coded
# diagnoses, only. This time I will also permit diagnosis to be indicated by
# abnormal HbA1c (i.e. >48 mmol/mol). The justification is that clinically-coded
# dates of diabetes diagnosis can disagree with HbA1c values by more than 10
# years (doi: 10.1038/s41416-025-03136-9). The HbA1c values are superior
# indicators but the validity of HbA1c values decreases as we go further back in
# the record because they weren't used diagnostically until a while after
# introduction. Therefore, in the early days, we will have to trust the
# clinically-coded date.
#
# In an email sent 26th July 2024, CB suggested a three-option algorthim for
# identifying the date of diagnosis assuming a `date_diagnosis_threshold` of
# "2000-01-01":
#
# 1. If the clinically-coded diagnosis is before April 2004 AND there are raised
#    HbA1c values after April 2003 (note the difference in years), then use the
#    clinically-coded date.
#    - The actual statement from CB was: if the clinically-coded diagnosis is
#       before April 2004 AND some "recent" raised HbA1c, then use the
#       clinically-coded date, on the assumption that the date is not mis-coded.
#       This requires a threshold for "recent" (or does he mean "concurrent"?).
# 2. If the clinically-coded diagnosis is after April 2004 AND there are raised
#    HbA1c values before April 2003 (note the difference in years), then use the
#    earliest raised HbA1c date.
# 3. If the clinically-coded diagnosis is after April 2004 AND the first raised
#    HbA1c value is after April 2003 (note the difference in years), then use
#    the clinically-coded date.
#
# The years 2003 and 2004 are relative to the assumed baseline of 2000, so
# reference to them will be calculated as
#       lubridate::year( date_diagnosis_threshold + lubridate::years( x ) )
# where x = {3, 4}, respectively.  CB's definition does not specify what happens
# when both the clinically-coded and serological-informed dates are before 2003.
# In these cases, I will choose the serologically-coded date.
#
# My approach to identifying the date of diagnosis will be to start with the
# original method of using clinical codes, and then only change the date if it
# satisfies option #2, above.
#
# First, I need to convert historic percentage values to up-to-date mmol/mol
# values (Formala taken from Sacks (2012; doi: 10.2337/dc12-1348). Then I will
# define the cohort of relevant records.
#
qry_srcode_tests <-
  qry_srcode %>%
  # Remove the rows with no `person_id`. This shouldn't need to be done but was
  # spotted as an issue because it broke code much later on.
  dplyr::filter( !is.na( person_id ) ) %>%
  dplyr::select( person_id, DateEvent, NumericValue, SNOMEDCode ) %>%
  # ## Convert A1c% values to mmol/mol values. The rationale for the following
  # ## syntax is in `RESHAPE_check_HbA1c_SNOMED_codes.ipynb`.
  # ## Apply transformations, where appropriate.
  dplyr::mutate(
    NumericValue = NumericValue %>% as.numeric()
    # ## ## The following IF-ELSE statement does two things. It allows any 
    # ## ## SNOMED code not in the list to pass through the filter. It also
    # ## ## allows any SNOMED code that is in the list to pass through only
    # ## ## if its value is greater 2.15.
    # ## ## I would have applied this filter in online as a `dplyr::case_when()`
    # ## ## within a  dplyr::filter()` but `dbplyr` does not like that syntax.
    ,keep_in_filter = dplyr::if_else(
      SNOMEDCode %in% codes_SNOMED_test_of_interest
      ,NumericValue > 2.15
      ,TRUE
    )
  ) %>% 
  dplyr::filter( keep_in_filter ) %>%  
  dplyr::select( -keep_in_filter ) %>%
  dplyr::mutate(
    NumericValue = 
      dplyr::if_else(
        ( SNOMEDCode %in% codes_SNOMED_test_of_interest ) & ( NumericValue < 20 )
        ,10.93 * ( NumericValue ) - 23.50
        # Sacks (2012) formula, for which there is no justification. It results
  ...
  dplyr::mutate(
    keep_in_filter = dplyr::if_else(
      SNOMEDCode %in% codes_SNOMED_test_of_interest
      ,dplyr::between( NumericValue, test_value_cutoff_lower, test_value_cutoff_upper )
      ,TRUE
    )
  ) %>%
  dplyr::filter( keep_in_filter ) %>%
...
qry_srcode_other <-
  ...
  dplyr::filter(
    ( !SNOMEDCode %in% codes_SNOMED_test_of_interest ) &
      ( !SNOMEDCode %in% codes_SNOMED_diagnoses_of_interest )
  )

qry_srcode <-
  qry_srcode_tests %>%
  dplyr::union_all( qry_srcode_diagnoses ) %>%
  dplyr::union_all( qry_srcode_other )

rm( qry_srcode_tests, qry_srcode_diagnoses, qry_srcode_other )
...
qry_records_with_T2DM_diagnoses_HbA1c <-
  ...
  dplyr::filter( as.numeric( NumericValue ) == max( as.numeric( NumericValue ), na.rm = TRUE ) ) %>%
  dplyr::ungroup() %>%
  # Tidy up.
...
qry_records_with_T2DM_diagnoses <-
  ...
  qry_records_with_T2DM_diagnoses_coded %>%
  dplyr::full_join( qry_records_with_T2DM_diagnoses_HbA1c, by = join_by( person_id ) ) %>%
...
{
  qry_records_with_T2DM_diagnoses <-
    dplyr::filter( qry_records_with_T2DM_diagnoses, date_coded_diagnosis <= date_diagnosis_threshold )
} 
qry_records_with_T2DM_diagnoses <-
  dplyr::select( qry_records_with_T2DM_diagnoses, person_id, date_diagnosis ) %>%
  dplyr::distinct()
rm( qry_records_with_T2DM_diagnoses_coded, qry_records_with_T2DM_diagnoses_HbA1c )

# I've opted to collect at this stage because maintaining the lazy table is
# causing problems.
df_records_with_T2DM_diagnoses <- qry_records_with_T2DM_diagnoses %>% dplyr::collect() 


# Retrieve dates of prescriptions in the follow-up period.
adjusted_date_followup_start <-
  date_followup_start -
  ifelse(
    is.null( HMA_adjust_lookBack_window )
    ,lubridate::weeks( 16 )
    ,HMA_adjust_lookBack_window
  )
df_log_prescription_longFormat <-
  # # We'll continue to work with the query for while because some up-coming
  # # joins are to tables that are quite large. We'll collect into a data.frame
  # # when things are more manageable.
  qry_records_with_T2DM_diagnoses %>%
  dplyr::left_join( qry_srprimarycaremedication, by = join_by( person_id ) ) %>% 
  # Filter for every record that has a prescription for any diabetes medication.
  dplyr::filter( NameOfMedication %in% vectorOf_names_meds_of_interest ) %>%
  ...
  # Collect into a data.frame to avoid the subsequent data processing getting
  # bunged up in SQL lazy tables.
  dplyr::collect() %>%
  # Replace the name of the medication in `event_value` with the short name created earlier.
  dplyr::left_join(
    df_shortNames_meds_of_interest
    ,by = join_by( NameOfMedication == drug_name )
  ...
  dplyr::distinct( person_id, date_diagnosis, start_dttm, event_value ) %>%
  # Filter for however many subsequent prescriptions were specified in
  # `n_iterations`, then number them.
  dplyr::group_by( person_id ) %>%
  ...
  dplyr::ungroup() %>%
  dplyr::mutate( keep_in_filter = i_prescrp <= (n_iterations + 1)
                 # The "+ 1" is added so that we always include the final
                 # "Unobserved" state.
  ) %>%
  dplyr::filter( keep_in_filter ) %>%
  dplyr::select( -keep_in_filter ) %>% 
...
df_log_test_longFormat <- 
  qry_srcode %>%
  collect() %>%
  dplyr::right_join( df_records_with_T2DM_diagnoses, by = join_by( person_id ) ) %>%
  # Filter records for only those that refer to the test of interest.
  dplyr::filter( SNOMEDCode %in% codes_SNOMED_test_of_interest ) %>% 
  # Extract the required fields.
  dplyr::distinct( person_id, DateEvent, NumericValue, testType ) %>%
  # Filter out test scores of 0, which are expected to be data entry
  # anomalies, given my selection of columns.
  dplyr::filter( NumericValue != "0" ) %>%
  # Filter for the last test on a given day, in the cases where multiple test
  # were taken on a given day.
  dplyr::mutate(
    datetime_of_DateEvent = lubridate::as_datetime( DateEvent )
    ,date_of_DateEvent = as.Date( DateEvent )
    ,DateEvent = as.POSIXct( DateEvent)
  ) %>%
  dplyr::group_by( person_id, date_of_DateEvent ) %>%
  dplyr::filter( datetime_of_DateEvent == max( datetime_of_DateEvent ) ) %>%
  dplyr::ungroup() %>% 
  # Filter for records within the follow-up period.
  dplyr::filter(
    dplyr::between( date_of_DateEvent, date_followup_start, date_followup_end )
  ) %>%
  # Rename columns.
  dplyr::rename(
    start_dttm = DateEvent
  ...
  # Filter for however many subsequent tests were specified in `n_iterations`
  # then number them.
  dplyr::group_by( person_id ) %>% 
  dplyr::arrange( start_dttm ) %>% 
  dplyr::filter( row_number() <= n_iterations + 1 ) %>%
  # The "+ 1" is added so that we always include the final
  # "Unobserved" state. 
  ...
  # Tidy up.
  dplyr::distinct( person_id, start_dttm, event_name, event_value, end_dttm, HbA1c, testType )


# Create a variable to indicate the date from which we no longer observe the
# patient in the record.
df_patients_of_interest_with_death_dates <-
  qry_srpatient %>% 
  dplyr::collect() %>%
  # # Select only those patients in whom we are interested.
  dplyr::inner_join( df_records_with_T2DM_diagnoses, by = join_by( person_id ) )
if( !is.na( dplyr::count( df_patients_of_interest_with_death_dates ) ) )
{
  df_date_unobserved <-
    df_patients_of_interest_with_death_dates %>%
...
df_log_PandT_longFormat <-
  df_log_prescription_longFormat %>%
  dplyr::mutate( HbA1c = NA_integer_, testType = NA_character_ ) %>%
  dplyr::union_all( df_log_test_longFormat ) %>%
  dplyr::ungroup() %>% 
  dplyr::distinct() %>%
  ...
  dplyr::ungroup() %>%
  # Replace the name of the medication in `event_value` with the short name created earlier.
  dplyr::left_join( df_shortNames_meds_of_interest, by = join_by( event_value == drug_name ) ) %>%
  dplyr::mutate( event_value = dplyr::if_else( is.na( drug_name_short ), event_value, drug_name_short ) ) %>%
...
df_log_multimorb_longFormat <-
  ...
  # Filter for multimorbidity diagnoses recorded outwith the multimorbidity
  # gap window of the date of diagnosis of the main diagnosis of interest.
  dplyr::mutate(
